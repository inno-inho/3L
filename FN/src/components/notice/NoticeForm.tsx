...
import { createNotice, getNotice, updateNotice, deleteNoticeFile } from '@/api/noticeApi';
...
const NoticeForm = () =>{
    ...
    // type NoticeFileType = File | {id:number; name:string; url:string};
    // const [files, setFiles] = useState<NoticeFileType[]>([]); // {name, url, id} 구조로 File[] 타입이 X
            ...
            if (res.files && res.files.length > 0){
                ...
                initExistingFiles(
                    res.files.map((f) => ({
                        ...
                        url: `api/notices/files/${f.id}/download`, // 다운로드, 뷰용 URL
                    }))
                );
            }
        }).catch(err => { console.error("공지 조회 실패", err); });
    }, [id, isEdit, initExistingFiles]);
    ...
    const handleSubmit = async () => {
        ...
        try{
            if (isEdit && id){
                ...
                await updateNotice(Number(id), noticeData, newFiles, deleteFileIds);
                setModalMessage("공지 수정이 완료되었습니다.");
                // setModalShow(true);
                navigate(`/notices/${id}`);
            } else{
                // 등록: 새 파일만 전달
                await createNotice(noticeData, newFiles); // 새로 등록
    ...
    const handleDeleteFile = async (fileId: number) => {
        console.log("삭제 클릭 : ", fileId);
        try{
            await deleteNoticeFile(fileId);
            // 화면에서 제거
            setFiles(prev => prev.filter(file => !(file instanceof File) && file.id !== fileId)); 
        } catch(e){
            alert("파일 삭제 실패");
        }
    }
