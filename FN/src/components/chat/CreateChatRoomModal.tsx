...
const CreateChatRoomModal = ({ show, onHide, onCreate }: CreateChatModalProps) => {
    ...
    const handleCreate = () => {
        if (selectedEmails.length === 0) {
            showAlert("초대 인원 부족", "최소 한 명 이상의 친구를 초대해주세요.");
            return;
        }
        if (isGroup && !roomName.trim()) {
            showAlert("입력 필요", "채팅방 이름을 입력해 주세요.");
            return
        }
                    ...
                    <div>
                        ...
                        <div className='max-h-64 overflow-auto space-y-2 pr-2 custom-scrollbar'>
                            {isLoading ? (
                                ...
                                <div className='text-center py-4 text-gray-400'>등록된 친구가 없습니다.</div>
                            ) : (
                                friends.map(friend => (       // 친구목록의 가짜 데이터임. 유저관련 기능 만들어지면 수정해야함
                                    <div
                                        key={friend.email}
                                    ...
                                    >
                                        <img src={friend.profileImageUrl} alt={friend.nickname} className='w-10 h-10 rounded-xl mr-3' />
